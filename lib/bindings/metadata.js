module.exports = function (Arrow, server) {
    ...
    return {
        ...
        fetchSchema: function fetchSchema(callback) {
            ...
            this.client.indices.status({ index: 'arrow:*' }, function (err, indexes) {
                if (err) {
                    return callback(err);
                }

                // Go through each index, one by one
                async.each(Object.keys(indexes.indices), function (name, callback) {

                    // Grab the mapping of the current index (mapping == schema)
                    this.client.indices.getMapping({ index: name }, function (err, results) {
                        if (err) {
                            return callback(err);
                        }
